...
@section('title', __('locale.themes'))
...
@section('content')
    ...
    <div class="card">
        <div class="d-flex justify-content-between align-items-center header-actions mx-2 row px-xxl-75 dataTables-Header">
            ...
            <div class="col-sm-12 col-md-6">
                <h3 class="link-dark">{{ __('locale.themes') }}</h3>
            </div>
            <div class="col-sm-12 col-md-6 d-flex justify-content-end">
                <a href="{{ route('themes.create') }}"
                    class="btn btn-flat-primary mb-1 btn-new">{{ __('locale.new-theme') }} <i data-feather='plus'></i></a>
            </div>
        </div>
        <div class="d-flex justify-content-center align-items-center header-actions row px-xxl-1 link-dark overflow-hidden">
            <hr class="link-dark" />
        </div>
        <input type="hidden" name="getJson" id="getJson" value="{{ route('themes.get-list-json') }}" />
        <input type="hidden" name="urlBase" id="urlBase" value="{{ route('themes.index') }}">
            ...
            <table class="user-list-table table">
                <thead class="table-light">
                    <tr>
                        ...
                        <th></th>
                        <th class="ps-md-4">
                            <div class="ps-md-2">{{ __('locale.name') }}</div>
                        </th>
                        <th class="">
                            <div class="">{{ __('locale.section') }}</div>
                        </th>
                        <th class="">
                            <div class="">{{ __('locale.indexed') }}</div>
                        </th>
                        <th></th>
                    </tr>
                </thead>
                ...
                <div class="modal-content" style="border-radius: 0!important;">
                    <div class="modal-body pb-md-4">
                        <div class="text-center pt-md-4 pb-md-3">
                            <h2>{{ __('locale.do-you-delete-theme') }}</h2>
    ...
    <form method="POST" action="" class="d-flex" id="form-delete">
        @csrf
        @method( 'DELETE' )
        <input type="hidden" name="urlDelete" id="urlDelete" value="{{ route('themes.destroy', [-1]) }}">
    </form>
    <!-- Vertical modal end-->
@endsection
                        ...
                        },
                        {
                            ...
                            render: function (data, type, full, meta) {
                                ...
                                if (data <= 9) {
                                    ...
                                    return (`<span class="hidden">${data}</span></span>`);
                                }
                            },
                        },
                        {
                        ...
                        },
                        {
                            ...
                            render: function(data, type, full, meta) {
                                ...
                                return (
                                    ...
                                    ${feather.icons['trash'].toSvg({ class: 'font-small-4 me-50' })} Borrar tema</button>` +
                                    '</div>' +
                                    '</div>' +
                                    '</div>'
                                );
                    ...
                    initComplete: function() {
                        ...
                        this.api()
                            ...
                            .every(function() {
                                var column = this;
                                var select = $(
                                        '<select id="role" class="form-select text-capitalize rounded-25 py-md-1 border-dark"><option value=""> Sección </option></select>'
                                    )
                                    .appendTo('.role')
                                    .on('change', function() {
                        ...
                        this.api()
                            ...
                            .every(function() {
                                var column = this;
                                var select = $(
                                        `<button type="button" class="btn btn-primary rounded-pill py-md-1"><span class="text-dark">Buscar</span></button>`
                                    )
                                    .appendTo('.search')
                                    .on('click', function() {
