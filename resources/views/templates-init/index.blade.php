...
@section('title', __('locale.init-template'))
...
@section('content')
    ...
    <div class="card">
        <div class="d-flex justify-content-between align-items-center header-actions mx-2 row px-xxl-75 dataTables-Header">
            ...
            <div class="col-sm-12 col-md-6">
                <h3 class="link-dark">{{ __('locale.init-template') }}</h3>
            </div>
            <div class="col-sm-12 col-md-6 d-flex justify-content-end">
                <a href="{{ route('templates-init.create') }}"
                    class="btn btn-flat-primary mb-1 btn-new">{{ __('locale.new-section') }} <i data-feather='plus'></i></a>
            </div>
        </div>
        <div class="d-flex justify-content-center align-items-center header-actions row px-xxl-1 link-dark overflow-hidden">
            <hr class="link-dark" />
        </div>
        <input type="hidden" name="getJson" id="getJson" value="{{ route('templates-init.get-json') }}" />
        <input type="hidden" name="urlBase" id="urlBase" value="{{ route('templates-init.index') }}">
            ...
            <table class="user-list-table table">
                <thead class="table-light">
                    <tr>
                        ...
                        <th></th>
                        <th class="col-11">
                            <div class="ps-md-4">{{ __('locale.h1') }}</div>
                        </th>
                        <th></th>
                    </tr>
                </thead>
                ...
                <div class="modal-content" style="border-radius: 0!important;">
                    <div class="modal-body pb-md-4">
                        <div class="text-center pt-md-4 pb-md-3">
                            <h2>{{ __('locale.do-you-delete-content-template') }}</h2>
    ...
    <form method="POST" action="" class="d-flex" id="form-delete">
        @csrf
        @method( 'DELETE' )
        <input type="hidden" name="urlDelete" id="urlDelete" value="{{ route('templates-init.destroy', [-1]) }}">
    </form>
    <!-- Vertical modal end-->
@endsection
                        ...
                        },
                        {
                            ...
                            render: function(data, type, full, meta) {
                                ...
                                return (
                                    ...
                                    ${feather.icons['trash'].toSvg({ class: 'font-small-4 me-50' })} Borrar plantilla</button>` +
                                    '</div>' +
                                    '</div>' +
                                    '</div>'
                                );
